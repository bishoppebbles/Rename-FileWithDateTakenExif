...
.NOTES
    Version 1.0.0
    Sam Pursglove
    Last modified: 08JUN2020
#>


[CmdletBinding()]
param (
    [Parameter(Position=0, Mandatory, ValueFromPipelineByPropertyName, HelpMessage='Directory(ies) containing files of interest')]
    [Alias('FullName','Name')]
    [string[]]$Path
)
...
Param(
    ...
    foreach($folder in $folders) { 
        ...
        foreach($file in $objFolder.items()) {  # retrieves a FolderItems object that represents the collection of items in the folder
            $FileMetaData = New-Object PSOBJECT 
            
            for($a ; $a -le 266; $a++) {  
                if($objFolder.getDetailsOf($file, $a)) { # retrieve the details about an item in the folder
                    $hash += @{
...
foreach ($pic in $pics) {
    ...
    if(($dateTaken = $pic.'Date taken') -ne $null) {    
        
        $picPath = $pic.Path.ToString()
        $picName = $pic.Name.Split('.')[0]
        $dateTaken = ($pic.'Date taken').toString().replace('/', '-').replace(':', '').replace(' ', '-')
        $newName = $dateTaken + '_' + $picName + '.jpg'
        
        Rename-Item -Path $picPath -NewName $newName

    } else {
        Write-Output "Skipping: the 'Date taken' EXIF field does not exist"
    }
}